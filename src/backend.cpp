...
QString defaultRoomName() {
  ...
  if (ownerName.isEmpty()) {
    ownerName = QCoreApplication::translate("Backend", "chunyuvpn");
  }
...
Backend::Backend(QObject *parent)
    : QObject(parent), steamReady_(false), localPort_(25565),
      localBindPort_(25565), lastTcpClients_(0), lastMemberLogCount_(-1),
      roomName_(QCoreApplication::translate("Backend", "chunyuvpn 房间")),
  ...
  QSettings settings;
  darkThemeEnabled_ = settings.value("ui/darkTheme", true).toBool();
  chatReminderEnabled_ =
      settings.value("ui/chatReminderEnabled", chatReminderEnabled_).toBool();
  chatNotificationMode_ =
      settings.value("ui/chatNotificationMode", chatNotificationMode_).toInt();
  if (chatNotificationMode_ < 0 || chatNotificationMode_ > 2) {
    chatNotificationMode_ = 0;
  }
  int storedMode =
      settings
          .value("ui/defaultConnectionMode",
                 static_cast<int>(ConnectionMode::Tun))
          .toInt();
  ConnectionMode loadedMode = ConnectionMode::Tcp;
  if (storedMode == static_cast<int>(ConnectionMode::Tun)) {
    loadedMode = ConnectionMode::Tun;
  } else if (storedMode == static_cast<int>(ConnectionMode::Udp)) {
    loadedMode = ConnectionMode::Udp;
  }
  defaultConnectionMode_ = loadedMode;
  connectionMode_ = defaultConnectionMode_;

  double storedOpacity =
      settings.value("ui/cardBackgroundOpacity", cardBackgroundOpacity_)
          .toDouble();
  if (storedOpacity < 0.3 || storedOpacity > 1.0) {
    storedOpacity = 1.0;
  }
  cardBackgroundOpacity_ = storedOpacity;

  QString backgroundPath =
      settings.value(QStringLiteral("ui/customBackgroundImagePath")).toString();
...
Backend::~Backend() {
  ...
  if (steamManager_) {
    steamManager_->setUdpForwarder(nullptr);
  }
...
void Backend::showWindowsNotification(const QString &title,
                                      const QString &body) {
#ifdef Q_OS_WIN
  if (!trayIcon_) {
    ...
    if (icon.isNull()) {
      icon = QIcon(QStringLiteral(":/qt/qml/chunyuvpn/qml/ConnectTool/logo.ico"));
    }
    trayIcon_->setIcon(icon);
    trayIcon_->setToolTip(QStringLiteral("chunyu · vpn"));
    trayIcon_->show();
  }
...
bool Backend::tryInitializeSteam() {
  ...
  if (!steamReady_) {
    ...
    const bool skipPrompt =
        settings.value(QStringLiteral("steam/skipLaunchPrompt"), false)
            .toBool();
    if (!skipPrompt && !steamLaunchPromptShown_ && !isSteamRunning()) {
      steamLaunchPromptShown_ = true;
      emit steamInitFailedNoClient();
    }
#endif
...
void Backend::ensureServerRunning() {
  if (inUdpMode()) {
    ...
    const int bindPort = isHost() ? 0 : localBindPort_;
    udpForwarder_ = std::make_unique<UDPForwarder>(bindPort, localPort_, steamManager_.get());
    steamManager_->setUdpForwarder(udpForwarder_.get());
    udpForwarder_->setClientCountCallback([this](int count) {
      QMetaObject::invokeMethod(
          this,
          [this, count]() {
    ...
    if (!udpForwarder_->start()) {
      qWarning() << tr("启动本地 UDP 转发失败。");
      udpForwarder_.reset();
      lastTcpClients_ = 0;
    }
    emit serverChanged();
...
void Backend::setChatReminderEnabled(bool enabled) {
  ...
  QSettings settings;
  settings.setValue("ui/chatReminderEnabled", chatReminderEnabled_);
  emit chatReminderEnabledChanged();
}

void Backend::setChatNotificationMode(int mode) {
  int next = mode;
  if (next < 0 || next > 2) {
    next = 0;
  }
  if (chatNotificationMode_ == next) {
    return;
  }
  chatNotificationMode_ = next;
  QSettings settings;
  settings.setValue("ui/chatNotificationMode", chatNotificationMode_);
  emit chatNotificationModeChanged();
}

void Backend::setDarkThemeEnabled(bool enabled) {
  if (darkThemeEnabled_ == enabled) {
    return;
  }
...
void Backend::setCardBackgroundOpacity(double opacity) {
  if (opacity < 0.3) {
    opacity = 0.3;
  } else if (opacity > 1.0) {
    opacity = 1.0;
  }
  if (cardBackgroundOpacity_ == opacity) {
    return;
  }
  cardBackgroundOpacity_ = opacity;
  QSettings settings;
  settings.setValue("ui/cardBackgroundOpacity", cardBackgroundOpacity_);
  emit cardBackgroundOpacityChanged();
}

void Backend::setCustomBackgroundImage(const QUrl &url) {
  if (customBackgroundImage_ == url) {
    return;
  }
  customBackgroundImage_ = url;
  QString path;
  if (url.isLocalFile()) {
  ...
  QSettings settings;
  if (path.isEmpty()) {
    settings.remove(QStringLiteral("ui/customBackgroundImagePath"));
  } else {
    settings.setValue(QStringLiteral("ui/customBackgroundImagePath"), path);
  }
  emit customBackgroundImageChanged();
}

void Backend::setDefaultConnectionMode(int mode) {
  ConnectionMode next = ConnectionMode::Tcp;
  if (mode == static_cast<int>(ConnectionMode::Tun)) {
    next = ConnectionMode::Tun;
  } else if (mode == static_cast<int>(ConnectionMode::Udp)) {
    next = ConnectionMode::Udp;
  }
  if (defaultConnectionMode_ == next) {
    return;
  }
  defaultConnectionMode_ = next;
  QSettings settings;
  settings.setValue("ui/defaultConnectionMode",
                    static_cast<int>(defaultConnectionMode_));
  if (!isHost() && !isConnected()) {
    setConnectionMode(static_cast<int>(next));
  }
  emit defaultConnectionModeChanged();
}
...
void Backend::setConnectionMode(int mode) {
  ...
  if (next == ConnectionMode::Tcp || next == ConnectionMode::Udp) {
    stopVpn();
    vpnWanted_ = false;
  }
...
QUrl Backend::importBackgroundImage(const QUrl &source) {
  QUrl effective = source;
  if (!effective.isValid()) {
    return {};
  }

  QString localPath;
  if (effective.isLocalFile()) {
    localPath = effective.toLocalFile();
  } else {
    localPath = effective.toString();
  }
...
#ifdef Q_OS_WIN
  if (localPath.startsWith('/') && localPath.size() > 2 &&
      localPath[1].isLetter() && localPath[2] == QLatin1Char(':')) {
  ...
  QDir baseDir(appDataDir);
  if (!baseDir.exists() && !baseDir.mkpath(QStringLiteral("."))) {
    return {};
  }

  const QString backgroundsDirPath =
      baseDir.filePath(QStringLiteral("backgrounds"));
  QDir backgroundsDir(backgroundsDirPath);
  if (!backgroundsDir.exists() &&
      !backgroundsDir.mkpath(QStringLiteral("."))) {
    return {};
  }

  QString fileName = srcInfo.fileName();
  if (fileName.isEmpty()) {
    fileName = QStringLiteral("custom_background");
  }

  const QString destPath = backgroundsDir.filePath(fileName);
  ...
  QUrl result = QUrl::fromLocalFile(destPath);
  setCustomBackgroundImage(result);
  return result;
}

void Backend::optimizeMemory() {
#if defined(Q_OS_LINUX)
  malloc_trim(0);
#elif defined(Q_OS_WIN)
  _heapmin();
#endif
  updateStatusText_ = tr("已尝试优化内存使用。");
...
void Backend::handleChatMessage(uint64_t senderId, const QString &message) {
  ...
  if (!entry.isSelf && chatReminderEnabled_) {
    int mode = chatNotificationMode_;
    if (mode < 0 || mode > 2) {
      mode = 0;
    }
    const bool useInApp = (mode == 0 || mode == 2);
    const bool useWindows =
#ifdef Q_OS_WIN
        (mode == 1 || mode == 2);
#else
        false;
    ...
    if (useWindows) {
      const QString title =
          QCoreApplication::translate("Backend", "新聊天消息");
      const QString body =
          entry.displayName + QStringLiteral(": ") + entry.message;
...
void Backend::updateLobbiesList(
  ...
  for (const auto &lobby : lobbies) {
    ...
    const QString modeString = QString::fromStdString(lobby.mode);
    if (modeString == QStringLiteral("tun")) {
      entry.mode = tr("TUN 模式");
    } else if (modeString == QStringLiteral("udp")) {
      entry.mode = tr("UDP 模式");
    } else if (modeString == QStringLiteral("tcp")) {
      entry.mode = tr("TCP 模式");
    } else if (roomManager_) {
      const bool wantsTun = roomManager_->lobbyWantsTun(lobby.id);
      if (wantsTun) {
        entry.mode = tr("TUN 模式");
      }
    }
...
void Backend::checkForUpdates(bool useProxy) {
  ...
  const QString apiUrl = QStringLiteral(
      "https://api.github.com/repos/liangcka/chunyu-vpn/releases/latest");

  const QUrl url(apiUrl);
  QNetworkRequest req{url};
  req.setHeader(QNetworkRequest::UserAgentHeader,
                QStringLiteral("chunyuvpn"));
...
void Backend::installDownloadedUpdate() {
  if (downloadSavedPath_.isEmpty()) {
    updateStatusText_ = tr("请先下载更新。");
    emit updateInfoChanged();
    return;
  }

  const QFileInfo downloaded(downloadSavedPath_);
  if (!downloaded.exists() || !downloaded.isFile()) {
    updateStatusText_ = tr("找不到已下载的更新文件：%1").arg(downloadSavedPath_);
    emit updateInfoChanged();
    return;
  }

  const QString suffix = downloaded.suffix().toLower();
  if (suffix == QStringLiteral("exe") || suffix == QStringLiteral("msi")) {
    updateStatusText_ = tr("正在启动安装程序…");
    emit updateInfoChanged();
    const bool started = QProcess::startDetached(downloaded.absoluteFilePath(), {});
    if (!started) {
      updateStatusText_ = tr("无法启动安装程序：%1").arg(downloaded.absoluteFilePath());
      emit updateInfoChanged();
      return;
    }
    QMetaObject::invokeMethod(QCoreApplication::instance(), []() {
  ...
  if (suffix == QStringLiteral("zip")) {
    ...
    const QString tempDir =
        QDir(tempRoot).filePath(QStringLiteral("chunyuvpn_update_%1")
                                    .arg(QDateTime::currentDateTimeUtc()
                                             .toString(QStringLiteral("yyyyMMddhhmmss"))));

    const qint64 targetPid = QCoreApplication::applicationPid();

    QString command;
    command += QStringLiteral("$ErrorActionPreference='Stop'; ");
    command += QStringLiteral("$zip=%1; ").arg(quotePs(zipPath));
    command += QStringLiteral("$tmp=%1; ").arg(quotePs(tempDir));
    command += QStringLiteral("$app=%1; ").arg(quotePs(appDir));
    command += QStringLiteral("$exe=%1; ").arg(quotePs(exeName));
    command += QStringLiteral("$targetPid=%1; ").arg(targetPid);
    command += QStringLiteral("if (Test-Path -LiteralPath $tmp) { Remove-Item -LiteralPath $tmp -Recurse -Force }; ");
    command += QStringLiteral("New-Item -ItemType Directory -Path $tmp | Out-Null; ");
    command += QStringLiteral("Expand-Archive -LiteralPath $zip -DestinationPath $tmp -Force; ");
    command += QStringLiteral("$candidate = Get-ChildItem -Path $tmp -Filter $exe -Recurse -File | Select-Object -First 1; ");
    command += QStringLiteral("if (-not $candidate) { exit 11 }; ");
    command += QStringLiteral("$src = $candidate.Directory.FullName; ");
    command += QStringLiteral("Wait-Process -Id $targetPid; ");
    command += QStringLiteral("Copy-Item -Path (Join-Path $src '*') -Destination $app -Recurse -Force; ");
    command += QStringLiteral("Start-Process -FilePath (Join-Path $app $exe); ");

    updateStatusText_ = tr("正在安装更新…");
    emit updateInfoChanged();
    ...
    if (!started) {
      updateStatusText_ = tr("无法启动更新进程，请手动解压覆盖安装目录。\n%1")
                              .arg(downloaded.absoluteFilePath());
      emit updateInfoChanged();
      return;
    }
    QMetaObject::invokeMethod(QCoreApplication::instance(), []() {
  ...
  updateStatusText_ = tr("无法自动安装该更新文件，请手动处理：%1")
                          .arg(downloaded.absoluteFilePath());
  emit updateInfoChanged();
  QDesktopServices::openUrl(QUrl::fromLocalFile(downloaded.absolutePath()));
}
...
void Backend::handleUpdateReply() {
  ...
  for (const auto &assetVal : assets) {
    ...
    if (lower.endsWith(QStringLiteral(".exe"))) {
      score += 100;
    } else if (lower.endsWith(QStringLiteral(".msi"))) {
      score += 95;
    ...
    if (lower.contains(QStringLiteral("mac")) ||
        lower.contains(QStringLiteral("macos")) ||
    ...
    if (lower.endsWith(QStringLiteral(".appimage"))) {
      score += 100;
    } else if (lower.endsWith(QStringLiteral(".tar.gz")) ||
