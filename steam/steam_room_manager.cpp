...
void SteamMatchmakingCallbacks::OnLobbyListReceived(LobbyMatchList_t *pCallback,
  ...
  for (uint32 i = 0; i < pCallback->m_nLobbiesMatching; ++i) {
    ...
    const char *mode =
        SteamMatchmaking()->GetLobbyData(lobbyID, kLobbyKeyMode);
    if (mode && mode[0] != '\0') {
      info.mode = mode;
    }
  ...
  if (current.IsValid() && roomManager_->publishLobby_) {
    ...
    if (!exists) {
      ...
      const char *mode =
          SteamMatchmaking()->GetLobbyData(current, kLobbyKeyMode);
      if (mode && mode[0] != '\0') {
        info.mode = mode;
      }
...
void SteamRoomManager::refreshLobbyMetadata() {
  ...
  const char *modeStr = kLobbyModeTcp;
  if (wantsTun) {
    modeStr = kLobbyModeTun;
  } else if (advertisedIsUdp_) {
